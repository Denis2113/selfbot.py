...
class Trivia:
    ...
    async def triviaset(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            msg = box("Primesc eo Puncte?: {BOT_PLAYS}\n"
                      "Secunde sa Raspunzi: {DELAY}\n"
                      "Puncte sa castigi: {MAX_SCORE}\n"
                      "Dau raspunsul?: {REVEAL_ANSWER}\n"
                      "".format(**settings))
    ...
    async def maxscore(self, ctx, score : int):
        ...
        if score > 0:
            ...
            self.save_settings()
            await self.bot.say("Punctele necesare pt{}".format(score))
        else:
    ...
    async def botplays(self, ctx):
        ...
        if self.settings[server.id]["BOT_PLAYS"]:
            ...
            await self.bot.say("HAHA,Nabule voi castiga dinou.")
        else:
            self.settings[server.id]["BOT_PLAYS"] = True
            await self.bot.say("Eu voi primi un punct cand tu nu stii ceva.")
        self.save_settings()
    ...
    async def revealanswer(self, ctx):
        ...
        if self.settings[server.id]["REVEAL_ANSWER"]:
            ...
            await self.bot.say("Nu voi mai da raspunsurile.")
        else:
            self.settings[server.id]["REVEAL_ANSWER"] = True
            await self.bot.say("Iti dau eo raspunsurile cand nu stii.")
        self.save_settings()
    ...
    async def trivia(self, ctx, list_name: str):
        ...
        if not session:
            ...
            await self.bot.say("Deja este un trivia pornit.")
    ...
    async def trivia_stop(self, ctx):
        ...
        if session:
            if author == session.starter or is_authorized:
                ...
                await self.bot.say("Trivia Oprit.")
            else:
                await self.bot.say("NU AI VOIE SA FACI ASTA.")
        else:
            await self.bot.say("Nicio trivia aici.")
    ...
    async def trivia_list(self):
        ...
        if lists:
            ...
            await self.bot.say("Nici o intrebare.")
...
class TriviaSession():
    def __init__(self, bot, trivia_list, message, settings):
        self.bot = bot
        self.reveal_messages = ("Eu o stiu pe asta {}!",
                                "EZ PZ BOY: {}.",
                                "SERIOS, ma? raspunsu e: {} .")
        self.fail_messages = ("Alta Intrebare...",
                              "astaa...",
                              "Sunt sigur ca stii raspunsul la asta.",
    ...
    async def new_question(self):
        ...
        while self.status != "correct answer" and abs(self.timer - int(time.perf_counter())) <= self.settings["DELAY"]:
            if abs(self.timeout - int(time.perf_counter())) >= self.settings["TIMEOUT"]:
                await self.bot.say("Bajeti...? Cred ca ma opresc aici.")
    ...
    async def check_answer(self, message):
        ...
        if has_guessed:
            ...
            msg = "Bravo ma {}! **+1** pt tine!".format(message.author.name)
