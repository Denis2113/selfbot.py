...
class Mod:
    ...
    async def kick(self, ctx, user: discord.Member, *, reason: str = None):
        ...
        if author == user:
            await self.bot.say("Sa te ranesti singur nu e bine "
                               "nabule \N{PENSIVE FACE}")
            return
        elif not self.is_allowed_by_hierarchy(server, author, user):
            await self.bot.say("Nu esti destul de smeker sa faci "
                               "asta aici"
                               "nabule.")
            return
        ...
        try:
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("Gata, Huh, ma simt bine.")
        except discord.errors.Forbidden:
            await self.bot.say("Nu am voie.")
    ...
    async def ban(self, ctx, user: discord.Member, days: str = None, *, reason: str = None):
        ...
        if days < 0 or days > 7:
            await self.bot.say("Cate?. trebuie sa fie intre 0 si 7.")
            return

        try:
            self.temp_cache.add(user, server, "BAN")
            await self.bot.ban(user, days)
            logger.info("{}({}) la banat pe {}({}), stergand {} zile pline cu mesaje".format(
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("Gata, el chiar merita ban.")
        except discord.errors.Forbidden:
            await self.bot.say("Nu am voie.")
        except Exception as e:
            print(e)
    ...
    async def hackban(self, ctx, user_id: int, *, reason: str = None):
        ...
        if is_banned:
            await self.bot.say("Deja are ban.")
            return
    ...
    async def server_mute(self, ctx, user : discord.Member, *, reason: str = None):
        ...
        if not self.is_allowed_by_hierarchy(server, author, user):
            await self.bot.say("Nu te las sa faci asta "
                               "nu esti destul de smeker aici  "
                               "-Smakaroane ")
            return
        ...
        for channel in server.channels:
            ...
            try:
                ...
                await self.bot.say("Scuze nu am putut pentru ca nu am permisiunea "
                                   "sau acel membru este prea smeker pentru a primi mute "
                                   "incearca m-ai tazriu.")
                return
            else:
                await asyncio.sleep(0.1)
        if not register:
            await self.bot.say("Acel membru deja are mute.")
            return
        ...
        await self.new_case(server,
                            ...
                            user=user,
                            reason=reason)
        await self.bot.say("Gata are mute.")
    ...
    async def channel_unmute(self, ctx, user : discord.Member):
        ...
        if overwrites.send_messages:
            await self.bot.say("Ala nu cred ca are mute "
                               "aici.")
            return
        elif not self.is_allowed_by_hierarchy(server, author, user):
            await self.bot.say("Nu fa asta, nu esti destul de smeker "
                               "asa ca asteapta "
                               "-Smakaroane.")
            return
