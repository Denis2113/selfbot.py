...
class Alias:
    ...
    async def _add_alias(self, ctx, command, *, to_execute):
        ...
        if len(command.split(" ")) != 1:
            await self.bot.say("Nu pot sa fac cuvinte cu mai multe pseudonime"
                               "Eu permit argumente cu"
                               " Pseudionime. Stiu nu-i fain.")
            return
        if self.part_of_existing_command(command, server.id):
            await self.bot.say('Eu nu pot sa adaug un pseudonim care incepe cu '
                               'o comanda sau alt pseudonim, Scz <3')
            return
        ...
        if command not in self.bot.commands:
            ...
            await self.bot.say("Pseudonimul '{}' a fost adaugat.".format(command))
        else:
            await self.bot.say("Nu pot adauga '{}' pentru ca e o comanda "
                               "de la un bot.".format(command))
    ...
    async def _help_alias(self, ctx, command):
        ...
        if server.id in self.aliases:
            ...
            if command in server_aliases:
                ...
                await self.bot.say("Pseudonimu ala nu exista.")
    ...
    async def _del_alias(self, ctx, command):
        ...
        if server.id in self.aliases:
            ...
            dataIO.save_json(self.file_path, self.aliases)
        await self.bot.say("Pseudonimul '{}' a fost sters.".format(command))
        ...
        if server.id in self.aliases:
            ...
            for alias in sorted(self.aliases[server.id]):
                if len(message) + len(alias) + 3 > 2000:
                    ...
                    message = "```\n"
                message += "\t{}\n".format(alias)
            if message != "```Lista pseudonimelor:\n":
                message += "```"
                await self.bot.whisper(message)
            else:
                await self.bot.say("Nici un pseudonim aici.")
