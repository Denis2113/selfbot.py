...
class CustomCommands:
    ...
    async def cc_add(self, ctx, command : str, *, text):
        ...
        if command not in cmdlist:
            ...
            await self.bot.say("Comanda asta exista.Foloseste "
                               "`{}customcom edit` sa o editezi."
                               "".format(ctx.prefix))
    ...
    async def cc_edit(self, ctx, command : str, *, text):
        ...
        if server.id in self.c_commands:
            ...
            if command in cmdlist:
                ...
                self.c_commands[server.id] = cmdlist
                dataIO.save_json(self.file_path, self.c_commands)
                await self.bot.say("Am editato, ce mai astepti?.")
            else:
                await self.bot.say("Nu exista asa cv. foloseste "
                                   "`{}customcom add` sa o adaugi."
                                   "".format(ctx.prefix))
        else:
            await self.bot.say("Nici o comanda facuta pe aici."
                               " foloseste `{}customcom add` sa adaugi cateva."
                               "".format(ctx.prefix))
    ...
    async def cc_delete(self, ctx, command : str):
        ...
        if server.id in self.c_commands:
            ...
            if command in cmdlist:
                ...
                self.c_commands[server.id] = cmdlist
                dataIO.save_json(self.file_path, self.c_commands)
                await self.bot.say("gata wai am sterso.")
            else:
                await self.bot.say("Wai, daca nu exista nu pot.")
        else:
            await self.bot.say("Nici o comanda facuta pe aici."
                               " foloseste `{}customcom add`sa bagi cateva."
                               "".format(ctx.prefix))
